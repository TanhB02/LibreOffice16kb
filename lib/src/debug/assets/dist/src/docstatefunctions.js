...
window.addEventListener('load', function () {
    app.calc.cellCursorRectangle = new app.definitions.simpleRectangle(0, 0, 0, 0);
...
app.getViewRectangles = function () {
    if (app.map._docLayer._splitPanesContext)
        ...
        return [app.file.viewedRectangle.clone()];
};
// ToDo: _splitPanesContext should be an app variable.
app.isPointVisibleInTheDisplayedArea = function (twipsArray /* x, y */) {
    if (app.map._docLayer._splitPanesContext) {
        var rectangles = app.map._docLayer._splitPanesContext.getViewRectangles();
        for (var i = 0; i < rectangles.length; i++) {
            if (rectangles[i].containsPoint(twipsArray))
                return true;
        }
        return false;
    }
...
app.isXVisibleInTheDisplayedArea = function (twipsX) {
    if (app.map._docLayer._splitPanesContext) {
        var rectangles = app.map._docLayer._splitPanesContext.getViewRectangles();
        for (var i = 0; i < rectangles.length; i++) {
            if (rectangles[i].containsX(twipsX))
                return true;
        }
        return false;
    }
...
app.isYVisibleInTheDisplayedArea = function (twipsY) {
    if (app.map._docLayer._splitPanesContext) {
        var rectangles = app.map._docLayer._splitPanesContext.getViewRectangles();
        for (var i = 0; i < rectangles.length; i++) {
            if (rectangles[i].containsY(twipsY))
                return true;
        }
        return false;
    }
...
app.isRectangleVisibleInTheDisplayedArea = function (twipsArray /* x, y, width, height */) {
    if (app.map._docLayer._splitPanesContext) {
        var rectangles = app.map._docLayer._splitPanesContext.getViewRectangles();
        for (var i = 0; i < rectangles.length; i++) {
            if (rectangles[i].intersectsRectangle(twipsArray))
                return true;
        }
        return false;
    }
...
app.isXOrdinateInFrozenPane = function (pixelX) {
    if (app.map._docLayer._splitPanesContext) {
        var splitPos = app.map._docLayer._splitPanesContext.getSplitPos();
        if (pixelX < splitPos.x)
            return true;
        else
            return false;
    }
...
app.registerExportFormat = function (label, format) {
    ...
    for (var i = 0; i < app.file.exportFormats.length; i++) {
        if (app.file.exportFormats[i].label == label &&
...
app.setFollowingEditor = function (viewId) {
    if (viewId === void 0) { viewId = -1; }
...
app.updateFollowingUsers = function () {
    ...
    if (isCellCursorVisible || isTextCursorVisible) {
        ...
        var cursorPositionInView = app.isPointVisibleInTheDisplayedArea(twipsArray);
        if (parseInt(app.getFollowedViewId()) ===
            parseInt(app.map._docLayer._viewId) &&
            !cursorPositionInView) {
            app.setFollowingOff();
        }
        else if (parseInt(app.getFollowedViewId()) === -1 &&
            cursorPositionInView) {
...
app.calc.isRTL = function () {
    if (!app.map._docLayer || !app.map._docLayer._lastStatusJSON)
        return false;
    var part = app.map._docLayer._lastStatusJSON.parts[app.map._docLayer._selectedPart];
    if (part)
...
app.calc.isPartHidden = function (part) {
    if (!app.map._docLayer || !app.map._docLayer._lastStatusJSON)
        return false;
    if (part >= app.map._docLayer._lastStatusJSON.parts.length)
        return false;
...
app.calc.isAnyPartHidden = function () {
    if (!app.map._docLayer || !app.map._docLayer._lastStatusJSON)
        return false;
    for (var i = 0; i < app.map._docLayer._lastStatusJSON.parts.length; i++) {
...
app.calc.getVisiblePartCount = function () {
    ...
    var count = 0;
    for (var i = 0; i < app.map._docLayer._lastStatusJSON.parts.length; i++) {
        if (app.map._docLayer._lastStatusJSON.parts[i].visible === 1)
            count++;
    }
    return count;
};
app.calc.getHiddenPartNameArray = function () {
    ...
    var array = [];
    for (var i = 0; i < app.map._docLayer._lastStatusJSON.parts.length; i++) {
        var part = app.map._docLayer._lastStatusJSON.parts[i];
        if (part.visible === 0)
            array.push(part.name);
    }
    return array;
};
app.impress.isSlideHidden = function (index) {
    if (app.impress.partList) {
        if (app.impress.partList.length > index)
            ...
            console.warn('Index is bigger than the part count (isSlideHidden): ' + index);
            return true;
        }
    }
    else
...
app.impress.areAllSlidesHidden = function () {
    if (app.impress.partList) {
        for (var i = 0; i < app.impress.partList.length; i++) {
            if (app.impress.partList[i].visible === 1)
                return false;
        }
        return true;
    }
...
app.impress.getSelectedSlidesCount = function () {
    ...
    if (app.impress.partList) {
        for (var i = 0; i < app.impress.partList.length; i++) {
            if (app.impress.partList[i].selected === 1)
                count++;
        }
    }
    return count;
...
app.impress.getIndexFromSlideHash = function (hash) {
    if (app.impress.partList) {
        for (var i = 0; i < app.impress.partList.length; i++) {
            if (app.impress.partList[i].hash === hash)
                return i;
        }
...
//# sourceMappingURL=docstatefunctions.js.map