...
app.definitions.Socket = L.Class.extend({
    ...
    connect: function (socket) {
        ...
        if (map.options.docParams.access_token && parseInt(map.options.docParams.access_token_ttl)) {
            ...
            this._accessTokenExpireTimeout = setTimeout(L.bind(this._sessionExpiredWarning, this), parseInt(map.options.docParams.access_token_ttl) - Date.now() - tokenExpiryWarning);
        }
    ...
    _sessionExpiredWarning: function () {
        ...
        this._accessTokenExpireTimeout = setTimeout(L.bind(this._sessionExpiredWarning, this), 120 * 1000);
    },
    ...
    _logSocket: function (type, msg) {
        ...
        window.app.console.log(+new Date() + ' %c' + type + status + '%c: ' + msg.concat(' ').replace(' ', '%c '), 'background:#ddf;color:black', color, 'color:');
    },
    ...
    _emitSlurpedEvents: function () {
        ...
        var completeEventWholeFunction = this.createCompleteTraceEvent('emitSlurped-' + String(queueLength), { '_slurpQueue.length': String(queueLength) });
            ...
            for (var i = 0; i < queueLength; ++i) {
                ...
                if (evt.isComplete()) {
                    ...
                    try {
                        ...
                        var msg = 'Exception ' + e + ' emitting event ' + evt.data + '\n' + e.stack;
                        window.app.console.error(msg);
                        // When debugging let QA know something is up.
                        if (window.enableDebug || L.Browser.cypressTest)
                            this._map.uiManager.showInfoModal('cool_alert', '', msg, '', _('Close'), function () { }, false);
        ...
        if (this._map) {
            var completeCallback = function () {
                ...
                _this._map.fire('messagesdone');
                _this._renderEventTimerStart = performance.now();
                _this._inLayerTransaction = false;
                if (_this._slurpDuringTransaction) {
                    _this._slurpDuringTransaction = false;
                    _this._queueSlurpEventEmission(1);
                }
            };
    ...
    _onMessage: function (e) {
        ...
        if (textMsg.startsWith('coolserver ')) {
            ...
            if (this.WSDServer && this.WSDServer.Id) {
                ...
                window.app.console.assert(this._map.options.wopiSrc === window.wopiSrc, 'wopiSrc mismatch!: ' + this._map.options.wopiSrc + ' != ' + window.wopiSrc);
            ...
            if (parseInt(h, 16).toString(16) === h.toLowerCase().replace(/^0+/, '')) {
                ...
                var versionContainer = document.getElementById('coolwsd-version');
            ...
            var versionContainer = document.getElementById('lokit-version');
            versionContainer.replaceChildren();
    ...
    _renameOrSaveAsCallback: function (textMsg, command) {
        ...
        if (command !== undefined && command.url !== undefined && command.url !== '') {
            ...
            if (textMsg.startsWith('renamefile:')) {
                ...
                var noAuthHeader = this._getParameterByName(url, 'no_auth_header');
        ...
        result.finish = function () {
            if (this.active) {
                ...
                that.sendTraceEvent(name, 'X', 'ts=' + Math.round(this.begin * 1000) +
                    ' dur=' + Math.round((now - this.begin) * 1000), args);
                this.active = false;
            }
        };
        result.abort = function () {
