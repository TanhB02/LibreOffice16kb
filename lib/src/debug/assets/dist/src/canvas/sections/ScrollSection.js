...
var cool;
(function (cool) {
    var ScrollSection = /** @class */ (function (_super) {
        __extends(ScrollSection, _super);
        function ScrollSection(isRTL) {
            var _this = _super.call(this) || this;
            _this.name = L.CSections.Scroll.name;
            _this.processingOrder = L.CSections.Scroll.processingOrder;
            _this.drawingOrder = L.CSections.Scroll.drawingOrder;
            _this.zIndex = L.CSections.Scroll.zIndex;
            _this.windowSection = true; // This section covers the entire canvas.
            _this.pendingScrollEvent = null;
            _this.stepByStepScrolling = false; // quick scroll will move "page up/down" not "jump to"
            _this.map = L.Map.THIS;
            _this.isRTL = isRTL !== null && isRTL !== void 0 ? isRTL : (function () { return false; });
            _this.map.on('scrollby', _this.onScrollBy, _this);
            _this.map.on('scrollvelocity', _this.onScrollVelocity, _this);
            _this.map.on('handleautoscroll', _this.onHandleAutoScroll, _this);
            return _this;
        }
        ...
        ScrollSection.prototype.moveMapBy = function (cX, cY, reset) {
        ...
        ScrollSection.prototype.onHandleAutoScroll = function (e) {
            ...
            var mousePosX = this.isRTL() ? e.map._size.x - e.pos.x : e.pos.x;
            var mapLeft = this.isRTL() ? e.map._size.x - e.map._getTopLeftPoint().x : e.map._getTopLeftPoint().x;
        ...
        ScrollSection.prototype.calculateYMinMax = function () {
            var diff = Math.round(app.view.size.pY - this.containerObject.getDocumentAnchorSection().size[1]);
            if (diff >= 0) {
                this.sectionProperties.yMin = 0;
        ...
        ScrollSection.prototype.calculateXMinMax = function () {
            var diff = Math.round(app.view.size.pX - this.containerObject.getDocumentAnchorSection().size[0]);
            if (diff >= 0) {
                this.sectionProperties.xMin = 0;
        ...
        ScrollSection.prototype.drawVerticalScrollBar = function () {
            var scrollProps = this.getVerticalScrollProperties();
            var isDarkBackground = this.map.uiManager.isBackgroundDark();
            var docType = this.sectionProperties.docLayer._docType;
            ...
            if (this.sectionProperties.drawScrollBarRailway) {
                ...
                this.context.fillRect(startX, this.sectionProperties.yMin + this.sectionProperties.yOffset, this.sectionProperties.scrollBarRailwayThickness, this.sectionProperties.yMax - this.sectionProperties.yMin - this.sectionProperties.yOffset);
            }
            if (this.sectionProperties.animatingVerticalScrollBar) {
                this.context.globalAlpha = this.sectionProperties.currentAlpha;
            }
        ...
        ScrollSection.prototype.drawHorizontalScrollBar = function () {
            ...
            var startX = this.isRTL() ? docWidth - scrollProps.startX - sizeX : scrollProps.startX;
            if (this.sectionProperties.drawScrollBarRailway) {
                this.context.globalAlpha = this.sectionProperties.scrollBarRailwayAlpha;
                this.context.fillStyle = this.sectionProperties.scrollBarRailwayColor;
                this.context.fillRect(this.sectionProperties.xMin + this.sectionProperties.xOffset, startY, this.sectionProperties.xMax - this.sectionProperties.xMin - this.sectionProperties.xOffset, this.sectionProperties.scrollBarRailwayThickness);
            }
        ...
        ScrollSection.prototype.onAnimate = function (frameCount, elapsedTime) {
            if (this.sectionProperties.animatingScroll) {
                var lineHeight = this.containerObject.getScrollLineHeight();
                // Smoothness will be affected by Firefox bug #1967935
                var timeDelta = (elapsedTime - this.sectionProperties.lastElapsedTime) / (1000 / 60);
                var accel = lineHeight * ScrollSection.scrollAnimationAcceleration * timeDelta * app.dpiScale;
                var maxVelocity = lineHeight * ScrollSection.scrollAnimationMaxVelocity * timeDelta * app.dpiScale;
                var maxDelta = lineHeight * ScrollSection.scrollAnimationMaxDelta * app.dpiScale;
                ...
                for (var i = 0; i < 2; ++i) {
                    ...
                    var sign = this.sectionProperties.scrollAnimationAcc[i] > 0 ? 1 : -1;
                ...
                if (this.sectionProperties.scrollWheelDelta[0] !== 0) {
                    var delta = this.sectionProperties.scrollWheelDelta[0];
                ...
                if (this.sectionProperties.scrollWheelDelta[1] !== 0) {
                    var delta = this.sectionProperties.scrollWheelDelta[1];
            ...
            var animatingScrollbar = (this.sectionProperties.animatingHorizontalScrollBar
                || this.sectionProperties.animatingVerticalScrollBar) &&
                elapsedTime && (elapsedTime < this.sectionProperties.fadeOutDuration);
            var animatingScroll = this.sectionProperties.animatingScroll
                && this.sectionProperties.scrollAnimationAcc.reduce(function (a, x) { return a + x; }, 0) !== 0;
        ...
        ScrollSection.prototype.showVerticalScrollBar = function () {
            ...
            if (!this.sectionProperties.mouseIsOnVerticalScrollBar) {
                ...
                setTimeout(function () {
                    if (_this.sectionProperties.mouseIsOnVerticalScrollBar) {
                        _this.increaseScrollBarThickness();
                    }
                }, 100);
        ...
        ScrollSection.prototype.showHorizontalScrollBar = function () {
            ...
            if (!this.sectionProperties.mouseIsOnHorizontalScrollBar) {
                ...
                // Prevent Instant Mouse hover
                setTimeout(function () {
                    if (_this.sectionProperties.mouseIsOnHorizontalScrollBar) {
                        _this.increaseScrollBarThickness();
                    }
                }, 100);
                if (!this.containerObject.isDraggingSomething() && !window.mode.isDesktop())
        ...
        // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
        ScrollSection.prototype.isMousePointerSyncedWithVerticalScrollBar = function (scrollProps, position) {
        ...
        // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
        ScrollSection.prototype.isMousePointerSyncedWithHorizontalScrollBar = function (scrollProps, position) {
            // Keep this desktop-only for now.
            if (!window.mode.isDesktop())
                return true;
            var spacer = 0;
        ...
        ScrollSection.prototype.onMouseMove = function (position, dragDistance, e) {
            ...
            if (this.sectionProperties.clickScrollVertical && this.containerObject.isDraggingSomething()) {
                ...
                this.showHorizontalScrollBar();
                var scrollProps = this.getHorizontalScrollProperties();
                var diffX = dragDistance[0] - this.sectionProperties.previousDragDistance[0];
        ...
        */
        ScrollSection.prototype.quickScrollVertical = function (point, originalSign) {
            var _this = this;
            ...
            if (this.stepByStepScrolling) {
                ...
                if (this.sectionProperties.clickScrollVertical)
                    this.sectionProperties.quickScrollVerticalTimer = setTimeout(function () {
                        if (!originalSign || originalSign === sign) {
                            _this.quickScrollVertical(point, sign);
                        }
        ...
        /*
            ...
            we quickly scroll the document to that position.
        */
        ScrollSection.prototype.quickScrollHorizontal = function (point, originalSign) {
            var _this = this;
            // Desktop only for now.
            if (!window.mode.isDesktop())
                return;
            ...
            var docWidth = this.map.getPixelBoundsCore().getSize().x;
            var startX = this.isRTL() ? docWidth - props.startX - sizeX : props.startX;
            ...
            if (this.stepByStepScrolling) {
                ...
                if (this.sectionProperties.clickScrollHorizontal)
                    this.sectionProperties.quickScrollHorizontalTimer = setTimeout(function () {
                        if (!originalSign || originalSign === sign) {
                            _this.quickScrollHorizontal(point, sign);
                        }
                    }, this.sectionProperties.stepDuration);
            }
            else {
        ...
        ScrollSection.prototype.clearQuickScrollTimeout = function () {
            if (this.sectionProperties.quickScrollVerticalTimer) {
                clearTimeout(this.sectionProperties.quickScrollVerticalTimer);
        ...
        ScrollSection.prototype.onClick = function (point, e) {
            if (this.isAnimating && this.sectionProperties.animatingWheelScrollVertical)
                this.containerObject.stopAnimating();
        };
        ScrollSection.prototype.animateScroll = function (delta) {
            var lineHeight = this.containerObject.getScrollLineHeight();
            for (var i = 0; i < 2; ++i) {
        ...
        ScrollSection.prototype.onMouseWheel = function (point, delta, e) {
            ...
            if (Math.abs(delta[1]) > Math.abs(delta[0])) {
                ...
                hscroll = delta[0];
            var shouldAnimate = this.sectionProperties.animateWheelScroll
            ...
            if (shouldAnimate) {
                if (this.sectionProperties.scrollQuirks) {
                    ...
                    if (e.deltaMode !== WheelEvent.DOM_DELTA_PIXEL)
                        ...
                        var nowIsAccurate = performance.now() % 1 !== 0;
                        var hasFractionalComponent = (e.deltaX % 1 !== 0) || (e.deltaY % 1 !== 0);
                        var deltaMaybeDiscrete = Math.abs(e.wheelDelta) % 60 === 0;
            ...
            if (shouldAnimate)
                ...
                this.sectionProperties.scrollAnimationDisableTimeout =
                    setTimeout(function () { _this.sectionProperties.scrollAnimationDisableTimeout = null; }, ScrollSection.scrollDirectTimeoutMs);
        ...
        ScrollSection.scrollAnimationAcceleration = 0.2;
        ScrollSection.scrollAnimationMaxVelocity = 2.5;
        ScrollSection.scrollAnimationMaxDelta = 75;
        ScrollSection.scrollDirectTimeoutMs = 100;
        // The number of lines a scroll-wheel tick should travel
        ScrollSection.scrollWheelDelta = 20;
        return ScrollSection;
    }(CanvasSectionObject));
    cool.ScrollSection = ScrollSection;
})(cool || (cool = {}));
L.getNewScrollSection = function (isRTL) {
    return new cool.ScrollSection(isRTL);
};
//# sourceMappingURL=ScrollSection.js.map