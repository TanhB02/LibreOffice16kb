...
var PermTextureTransition = /** @class */ (function (_super) {
    __extends(PermTextureTransition, _super);
    function PermTextureTransition(transitionParameters) {
        var _this = _super.call(this, transitionParameters) || this;
        // prettier-ignore
        _this.permutation256 = [
        ...
        _this.permTexture = _this.createPermTexture();
        return _this;
    }
    PermTextureTransition.prototype.createPermTexture = function () {
        var gl = this.gl;
        var permTex = gl.createTexture();
        gl.bindTexture(gl.TEXTURE_2D, permTex);
        gl.texStorage2D(gl.TEXTURE_2D, 1, gl.RGBA8, 256, 256);
        var data = this.generatePermutationData();
    ...
    PermTextureTransition.prototype.generatePermutationData = function () {
        var a = new Uint8Array(256 * 256 * 4); // Multiplied by 4 for RGBA
        for (var y = 0; y < 256; y++) {
            for (var x = 0; x < 256; x++) {
                var base = (x + y * 256) * 4;
                var value = this.permutation256[(y + this.permutation256[x]) % 256];
                a[base] = value; // R
    ...
    PermTextureTransition.prototype.displayPermSlide_ = function () {
        this.gl.activeTexture(this.gl.TEXTURE2);
        this.gl.bindTexture(this.gl.TEXTURE_2D, this.permTexture);
        this.gl.uniform1i(this.gl.getUniformLocation(this.program, 'permTexture'), 2);
    };
    PermTextureTransition.prototype.displaySlides_ = function (t) {
        ...
        this.gl.drawArrays(this.gl.TRIANGLE_STRIP, 0, this.leavingPrimitives[0].vertices.length);
        this.displayPermSlide_();
    };
    return PermTextureTransition;
}(SimpleTransition));
SlideShow.PermTextureTransition = PermTextureTransition;
//# sourceMappingURL=PermTextureTransition.js.map