...
L.CalcTileLayer = L.CanvasTileLayer.extend({
    ...
    _onZoomRowColumns: function () {
        ...
        if (this.sheetGeometry) {
            this.sheetGeometry.setTileGeometryData(app.tile.size.x, app.tile.size.y, TileManager.tileSize);
        }
    ...
    _hasPartsCountOrNamesChanged: function (lastStatusJSON, statusJSON) {
        if (!lastStatusJSON)
            return true;
        if (lastStatusJSON.parts.length !== statusJSON.parts.length)
            return true;
        else {
            for (var i = 0; i < statusJSON.parts.length; i++) {
    ...
    _refreshPartNames: function (statusJSON) {
        this._partNames = [];
        for (var i = 0; i < statusJSON.parts.length; i++) {
            this._partNames.push(statusJSON.parts[i].name);
        }
    },
    _refreshPartHashes: function (statusJSON) {
        app.calc.partHashes = [];
        for (var i = 0; i < statusJSON.parts.length; i++) {
    ...
    _getMarginPropertiesForTheMap: function () {
        var rowHeaderSection = app.sectionContainer.getSectionWithName(L.CSections.RowHeader.name);
        var columnHeaderSection = app.sectionContainer.getSectionWithName(L.CSections.ColumnHeader.name);
        var rowGroupSection = app.sectionContainer.getSectionWithName(L.CSections.RowGroup.name);
        var columnGroupSection = app.sectionContainer.getSectionWithName(L.CSections.ColumnGroup.name);
        var scrollSection = app.sectionContainer.getSectionWithName(L.CSections.Scroll.name);
        var scrollBarThickness = scrollSection ? scrollSection.sectionProperties.scrollBarThickness : 0;
        var marginLeft = (rowHeaderSection ? rowHeaderSection.size[0] : 0) + (rowGroupSection ? rowGroupSection.size[0] : 0);
        var marginTop = (columnHeaderSection ? columnHeaderSection.size[1] : 0) + (columnGroupSection ? columnGroupSection.size[1] : 0);
        return { marginLeft: marginLeft, marginTop: marginTop, scrollBarThickness: scrollBarThickness };
    },
    _calculateNewCanvasAndMapSizes: function (documentContainerSize, availableSpace, marginLeft, marginTop, scrollBarThickness) {
        var newMapSize = availableSpace.slice();
        var newCanvasSize = documentContainerSize.slice();
        var fileSizePixels = app.file.size.pToArray();
        ...
        return { newMapSize: newMapSize, newCanvasSize: newCanvasSize };
    },
    ...
    _syncTileContainerSize: function () {
        ...
        var _a = this._getMarginPropertiesForTheMap(), marginLeft = _a.marginLeft, marginTop = _a.marginTop, scrollBarThickness = _a.scrollBarThickness;
        // Available for tiles section.
        var availableSpace = [documentContainerSize[0] - marginLeft - scrollBarThickness, documentContainerSize[1] - marginTop - scrollBarThickness];
        var _b = this._calculateNewCanvasAndMapSizes(documentContainerSize, availableSpace, marginLeft, marginTop, scrollBarThickness), newMapSize = _b.newMapSize, newCanvasSize = _b.newCanvasSize;
        var mapElement = document.getElementById('map'); // map's size = tiles section's size.
        var oldMapSize = [mapElement.clientWidth, mapElement.clientHeight];
        var widthIncreased = oldMapSize[0] < newMapSize[0];
        var heightIncreased = oldMapSize[1] < newMapSize[1];
        var sizeChanged = oldMapSize[0] !== newMapSize[0] || oldMapSize[1] !== newMapSize[1];
    ...
    _onStatusMsg: function (textMsg) {
        ...
        if (statusJSON.width && statusJSON.height && this._documentInfo !== textMsg) {
            var temp = this._lastStatusJSON ? Object.assign({}, this._lastStatusJSON) : null;
            this._lastStatusJSON = statusJSON;
            this._documentInfo = textMsg;
            var firstSelectedPart = (typeof this._selectedPart !== 'number');
            if (statusJSON.readonly)
    ...
    refreshViewData: function (coordinatesData, compatDataSrcOnly, sheetGeometryChanged) {
        ...
        if (sheetGeometryChanged || !this.sheetGeometry) {
            this.requestSheetGeometryData({ columns: updateCols, rows: updateRows });
            return;
        }
        if (this.sheetGeometry) {
    ...
    _adjustCanvasSectionsForLayoutChange: function () {
        ...
        if (sheetIsRTL && this._layoutIsRTL !== true) {
            ...
            this._setAnchor('rowGroupSection', rowGroupSection, [[L.CSections.CornerGroup.name, 'bottom', 'top'], 'right']);
            this._setAnchor('columnGroupSection', columnGroupSection, ['top', [L.CSections.CornerGroup.name, '-left', 'right']]);
            this._setAnchor('cornerHeaderSection', cornerHeaderSection, [[L.CSections.ColumnGroup.name, 'bottom', 'top'], [L.CSections.RowGroup.name, '-left', 'right']]);
            this._setAnchor('rowHeaderSection', rowHeaderSection, [[L.CSections.CornerHeader.name, 'bottom', 'top'], [L.CSections.RowGroup.name, '-left', 'right']]);
            this._setAnchor('columnHeaderSection', columnHeaderSection, [[L.CSections.ColumnGroup.name, 'bottom', 'top'], [L.CSections.CornerHeader.name, '-left', 'right']]);
            if (columnHeaderSection)
                columnHeaderSection.expand = ['left'];
            this._setAnchor('tilesSection', tilesSection, [[L.CSections.ColumnHeader.name, 'bottom', 'top'], [L.CSections.RowHeader.name, '-left', 'right']]);
    ...
    _handleSheetGeometryDataMsg: function (jsonMsgObj, differentSheet) {
        if (!this.sheetGeometry) {
            ...
            this.sheetGeometry = new L.SheetGeometry(jsonMsgObj, app.tile.size.x, app.tile.size.y, TileManager.tileSize, this._selectedPart);
    ...
    _calculateScrollForNewCellCursor: function () {
        ...
        var paneRectangles = app.getViewRectangles(); // SimpleRectangle array.
        var contained = false;
        for (var i = 0; i < paneRectangles.length; i++) {
        ...
        var freePane = paneRectangles[paneRectangles.length - 1]; // Last pane, this should be the scrollable - not frozen one.
        // Horizontal split
...
//# sourceMappingURL=CalcTileLayer.js.map