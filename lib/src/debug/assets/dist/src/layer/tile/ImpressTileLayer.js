...
L.ImpressTileLayer = L.CanvasTileLayer.extend({
    ...
    _onContextChange: function (e) {
        ...
        */
        var newContext = e.detail.context;
        var oldContext = e.detail.oldContext;
        var isDrawOrNotesPage = ['DrawPage', 'NotesPage'].includes(newContext);
        ...
        if (app.map.uiManager.getCurrentMode() === 'notebookbar' && isDrawOrNotesPage) {
            var targetElement = document.getElementById('notesmode');
            if (!targetElement)
                return;
...
//# sourceMappingURL=ImpressTileLayer.js.map