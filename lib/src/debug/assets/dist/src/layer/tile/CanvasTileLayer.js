...
L.TileSectionManager = L.Class.extend({
    ...
    _addSplitsSection: function () {
        var splitSection = new SplitSection();
    ...
    _getZoomDocPos: function (pinchCenter, pinchStartCenter, paneBounds, freezePane, splitPos, scale, findFreePaneCenter) {
        ...
        var minTopLeft = new L.Point(xMin, yMin);
        ...
        var docTopLeft = new L.Point(pinchStartCenter.x + (centerOffset.x - paneSize.x * panePortion.x) / scale, pinchStartCenter.y + (centerOffset.y - paneSize.y * panePortion.y) / scale);
        // Top left in document coordinates.
        var clampedDocTopLeft = new L.Point(Math.max(minTopLeft.x, docTopLeft.x), Math.max(minTopLeft.y, docTopLeft.y));
        var offset = clampedDocTopLeft.subtract(docTopLeft);
        ...
        var newPaneCenter = new L.Point((docTopLeft.x - splitPos.x + (paneSize.x + splitPos.x) * 0.5 / scale), (docTopLeft.y - splitPos.y + (paneSize.y + splitPos.y) * 0.5 / scale));
        return {
            offset: this._offset,
    ...
    _getZoomMapCenter: function (zoom) {
        ...
        return this._getZoomDocPos(this._newCenter, this._layer._pinchStartCenter, freePaneBounds, { freezeX: false, freezeY: false }, splitPos, scale, true /* findFreePaneCenter */).center;
    },
...
L.CanvasTileLayer = L.Layer.extend({
    ...
    _setup: function () {
        ...
        app.sectionContainer.addSection(L.getNewScrollSection(function () { return _this.isCalcRTL(); }));
        ...
        if (this._docType === 'spreadsheet') {
            var calcGridSection = new app.definitions.calcGridSection();
    ...
    _reset: function (hard) {
        var tileZoom = Math.round(this._map.getZoom()), tileZoomChanged = this._tileZoom !== tileZoom;
        if (hard || tileZoomChanged) {
            ...
            if (app.tile.size.x === 0 || app.tile.size.y === 0) {
                var tileWidthTwips = this.options.tileWidthTwips;
    ...
    _updateTileTwips: function () {
        ...
        var tileWidthTwips = Math.round(this.options.tileWidthTwips * factor);
        app.twipsToPixels = TileManager.tileSize / tileWidthTwips;
        app.pixelsToTwips = 1 / app.twipsToPixels;
        app.tile.size.pX = app.tile.size.pY = TileManager.tileSize;
        if (this._docType === 'spreadsheet')
    ...
    _checkSpreadSheetBounds: function (newZoom) {
        ...
        if (width < viewSize.x || height < viewSize.y) {
            ...
            this._map.setMaxBounds(new L.LatLngBounds(this._map.unproject(new L.Point(0, 0)), this._map.unproject(new L.Point(width * scale, height * scale))));
        }
    ...
    _isLatLngInView: function (position) {
        ...
        var positionInView = centerOffset.x > -viewHalf.x && centerOffset.x < viewHalf.x &&
    ...
    _twipsRectangleToPixelBounds: function (strRectangle) {
        ...
        return new L.Bounds(this._twipsToPixels(topLeftTwips), this._twipsToPixels(bottomRightTwips));
    },
    ...
    _createNewMouseEvent: function (type, inputEvent) {
        ...
        newEvent.initMouseEvent(type, true, true, window, 1, event.screenX, event.screenY, event.clientX, event.clientY, false, false, false, false, 0, null);
        return newEvent;
    },
    ...
    _onMessage: function (textMsg, img) {
        ...
        if (textMsg.startsWith('tile:') || textMsg.startsWith('delta:')) {
            ...
            var textMsgContent = textMsg.substr(22);
            var textMsgHtml = '';
            var textMsgPlainText = '';
            if (textMsgContent.startsWith('{')) {
                // Multiple formats: JSON.
                var textMsgJson = JSON.parse(textMsgContent);
            ...
            var hyperlinkTextBox = document.getElementById('hyperlink-text-box');
            if (hyperlinkTextBox) {
                // Hyperlink dialog is open, the text selection is for the link text
                // widget.
                var extracted = this._map.extractContent(textMsgHtml);
                hyperlinkTextBox.value = extracted.trim();
                var hyperlinkLinkBoxInput = document.getElementById('hyperlink-link-box-input');
            ...
            if (this._debug.overlayOn) {
                ...
                this._debug.setOverlayMessage('canonicalViewId', 'Canonical id changed to: ' + canonicalId + ' for view id: ' + viewId + ' with view renderend state: ' + viewRenderedState);
            }
            ...
            if (textMsg.startsWith('a11yfocuschanged:')) {
                ...
                this._map._textInput.onAccessibilityFocusChanged(obj.content, parseInt(obj.position), parseInt(obj.start), parseInt(obj.end), listPrefixLength, parseInt(obj.force) > 0);
            }
                ...
                this._map._textInput.onAccessibilityEditingInSelectionState(parseInt(obj.cell) > 0, parseInt(obj.enabled) > 0, obj.selection, obj.paragraph);
            }
            else if (textMsg.startsWith('a11yselectionchanged:')) {
                obj = JSON.parse(textMsg.substring('a11yselectionchanged:'.length + 1));
                this._map._textInput.onAccessibilitySelectionChanged(parseInt(obj.cell) > 0, obj.action, obj.name, obj.text);
            }
            ...
            var content_1 = JSON.parse(textMsg.substring('slidelayer:'.length + 1));
            ...
            var status_1 = textMsg.substring('sliderenderingcomplete:'.length + 1);
    ...
    _onInvalidateTilesMsg: function (textMsg) {
        ...
        var invalidArea = new app.definitions.simpleRectangle(command.x, command.y, command.width, command.height);
    ...
    _onCellFormulaMsg: function (textMsg) {
        ...
        var newFormula = textMsg.substring(13);
        if (this._lastFormula) {
            var minLength = Math.min(newFormula.length, this._lastFormula.length);
            var index = -1;
    ...
    _onCalcFunctionList: function (funcList, data) {
        ...
        for (var idx = 0; idx < funcList.length; ++idx) {
            ...
            entries.push(this._getCalcFunctionListEntry(name, undefined, func.index, func.signature, func.description));
        }
    },
    _onCalcFunctionListWithCategories: function (funcData, data) {
        ...
        for (idx = 0; idx < funcList.length; ++idx) {
            ...
            funcEntries.push(this._getCalcFunctionListEntry(name, func.category, func.index, func.signature, func.description));
        }
    },
    ...
    _onDownloadAsMsg: function (textMsg) {
        ...
        var url = window.makeHttpUrlWopiSrc('/' + this._map.options.urlPrefix + '/', this._map.options.doc, '/download/' + command.downloadid);
        this._map.hideBusy();
        if (this._map['wopi'].DownloadAsPostMessage) {
            ...
            if (this._map.options.print === false || L.Browser.cypressTest) {
                ...
                url = window.makeHttpUrlWopiSrc('/' + this._map.options.urlPrefix + '/', this._map.options.doc, '/download/' + command.downloadid, 'attachment=0');
    ...
    _onCellCursorMsg: function (textMsg) {
        ...
        if (textMsg.match('EMPTY')) {
            ...
            var offset = new L.Point(parseInt(strTwips[2]), parseInt(strTwips[3]));
            var bottomRightTwips = topLeftTwips.add(offset);
            var _cellCursorTwips = this._convertToTileTwipsSheetArea(new L.Bounds(topLeftTwips, bottomRightTwips));
            app.calc.cellAddress = new app.definitions.simplePoint(parseInt(strTwips[4]), parseInt(strTwips[5]));
            var tempRectangle = _cellCursorTwips.toRectangle();
    ...
    _onInvalidateViewCursorMsg: function (textMsg) {
        ...
        var username = this._map._viewInfo[viewId].username;
        var mode = obj.mode ? parseInt(obj.mode) : 0;
        var rectangle;
        if (obj.refpoint) {
            var refPoint = obj.refpoint.split(',');
    ...
    goToCellViewCursor: function (viewId) {
        if (OtherViewCellCursorSection.doesViewCursorExist(viewId)) {
            var viewCursorSection = OtherViewCellCursorSection.getViewCursorSection(viewId);
            ...
            if (!viewCursorSection.isVisible) {
                var scrollX_1 = viewCursorSection.position[0];
                var scrollY_1 = viewCursorSection.position[1];
                this.scrollToPos(new app.definitions.simplePoint(scrollX_1 * app.pixelsToTwips, scrollY_1 * app.pixelsToTwips));
            }
    ...
    _onViewCursorVisibleMsg: function (textMsg) {
        ...
        var section = app.definitions.otherViewCursorSection.getViewCursorSection(viewId);
        if (section) {
            var showCursor = obj.visible === 'true';
    ...
    _onTextViewSelectionMsg: function (textMsg) {
        ...
        if (rectArray.length) {
            ...
            var docLayer = this;
            this._viewSelections[viewId].pointSet = CPolyUtil.rectanglesToPointSet(rectangles, function (twipsPoint) {
                var corePxPt = docLayer._twipsToCorePixels(twipsPoint);
                corePxPt.round();
                return corePxPt;
            });
        ...
        for (var mark = 0; mark < marks.length; mark++) {
            ...
            if (strTwips != null) {
                ...
                for (var i = 0; i < strTwips.length; i += 4) {
                    ...
                    var boundsTwips = this._convertToTileTwipsSheetArea(new L.Bounds(topLeftTwips, topLeftTwips.add(offset)));
                ...
                if (scrollX !== 0 || scrollY !== 0) {
                    if (!this._map.wholeColumnSelected && !this._map.wholeRowSelected) {
                        ...
                        if (!this._isWholeColumnSelected(address) && !this._isWholeRowSelected(address)) {
                            var scroll_1 = new app.definitions.simplePoint(0, 0);
                            scroll_1.pX = scrollX;
                            scroll_1.pY = scrollY;
                            this.scrollByPoint(scroll_1);
                        }
                    }
                }
            }
    ...
    _onTextSelectionStartMsg: function (textMsg) {
        ...
        if (rectangles.length) {
            var topLeftTwips = rectangles[0].getTopLeft();
            var bottomRightTwips = rectangles[0].getBottomRight();
            var oldSelection = this._selectionHandles.start.rectangle ? this._selectionHandles.start.rectangle.clone() : null;
    ...
    _onCellSelectionAreaMsg: function (textMsg) {
        ...
        if (strTwips != null) {
            ...
            var adjustedTwipsWidth = boundsTwips.max.x - boundsTwips.min.x;
            var adjustedTwipsHeight = boundsTwips.max.y - boundsTwips.min.y;
    ...
    _postMouseEvent: function (type, x, y, count, buttons, modifier) {
        ...
        var verticalOffset = this.getFiledBasedViewVerticalOffset();
        if (verticalOffset) {
            y -= verticalOffset;
        }
    ...
    getFiledBasedViewVerticalOffset: function () {
        ...
        var additionPerPart = this._partHeightTwips + this._spaceBetweenParts;
        var verticalOffset = additionPerPart * this._selectedPart;
    ...
    postKeyboardEvent: function (type, charCode, unoKeyCode) {
        ...
        if (winId === 0) {
            ...
            app.socket.sendMessage('windowkey id=' + winId +
                ' type=' + type +
                ' char=' + charCode +
                ' key=' + unoKeyCode +
                '\n');
    ...
    scrollToPos: function (pos) {
        ...
        var needsXScroll = false;
        var needsYScroll = false;
        var CSSPixelsToTwips = app.dpiScale * app.pixelsToTwips;
        ...
        var controlYDown = center.y + (app.file.textCursor.visible ? app.file.textCursor.rectangle.cHeight :
            (app.calc.cellCursorVisible ? app.calc.cellCursorRectangle.cHeight : 0));
        var controlYUp = center.y - (app.file.textCursor.visible ? app.file.textCursor.rectangle.cHeight :
            (app.calc.cellCursorVisible ? app.calc.cellCursorRectangle.cHeight : 0));
        ...
        if (needsXScroll || needsYScroll) {
            var section = app.sectionContainer.getSectionWithName(L.CSections.Scroll.name);
            if (section) {
    ...
    goToViewCursor: function (viewId) {
        ...
        if (section && section.showSection) {
            var point = new app.definitions.simplePoint(section.position[0] * app.pixelsToTwips, section.position[1] * app.pixelsToTwips);
    ...
    _onUpdateTextViewSelection: function (viewId) {
        ...
        if (viewPointSet &&
            ...
            if (viewSelection) {
                ...
                viewSelection = new CSelections(viewPointSet, this._canvasOverlay, this._selectionsDataDiv, this._map, true /* isView */, viewId, true /* isText */);
    ...
    _onUpdateCellCursor: function (scrollToCursor, sameAddress) {
        ...
        if (app.calc.cellCursorVisible) {
            if (scrollToCursor &&
                ...
                var scroll_2 = this._calculateScrollForNewCellCursor();
                if (scroll_2.x !== 0 || scroll_2.y !== 0) {
                    var section = app.sectionContainer.getSectionWithName(L.CSections.Scroll.name);
                    if (section) {
                        section.moveMapBy(scroll_2.cX, scroll_2.cY, true);
                    }
                }
    ...
    _addCellDropDownArrow: function () {
        if (this._validatedCellAddress && app.calc.cellCursorVisible && this._validatedCellAddress.equals(app.calc.cellAddress.toArray())) {
            ...
            if (this.sheetGeometry) {
                ...
                var height = position.max.y - position.min.y;
            ...
            if (!app.sectionContainer.getSectionWithName('DropDownArrow')) {
                var dropDownSection = new app.definitions.calcValidityDropDown('DropDownArrow', position);
    ...
    _onUpdateCellResizeMarkers: function () {
        ...
        if (!selectionOnDesktop && (!this._cellCSelections.empty() || app.calc.cellCursorVisible)) {
            ...
            var posStart = new app.definitions.simplePoint(cellRectangle.x1, cellRectangle.y1);
            var posEnd = new app.definitions.simplePoint(cellRectangle.x2, cellRectangle.y2);
            var offset = this._cellSelectionHandleStart.sectionProperties.circleRadius;
    ...
    _updateMarkers: function () {
        ...
        var oldStart = this._selectionHandles.start.getPosition();
        var oldEnd = this._selectionHandles.end.getPosition();
        startPos.x -= 30 * app.dpiScale;
        this._selectionHandles.start.setPosition(startPos.x, startPos.y);
        var newStart = this._selectionHandles.start.getPosition();
        this._selectionHandles.end.setPosition(endPos.x, endPos.y);
        var newEnd = this._selectionHandles.end.getPosition();
        if (app.map._docLayer.isCalcRTL() && (newStart.y < newEnd.y || (newStart.y <= newEnd.y && newStart.x < newEnd.x))) {
        ...
        else if (!app.map._docLayer.isCalcRTL() &&
            ...
            */
            var temp = this._selectionHandles.start;
            ...
            for (var i = 0; i < this._previewInvalidations.length; i++) {
                ...
                for (var key in this._map._docPreviews) {
                    ...
                    if (preview.index >= 0) {
                        ...
                        var bounds = new L.Bounds(new L.Point(preview.tilePosX, preview.tilePosY), new L.Point(preview.tilePosX + preview.tileWidth, preview.tilePosY + preview.tileHeight));
            ...
            for (key in toInvalidate) {
                ...
                if (preview.autoUpdate) {
                    if (preview.index >= 0) {
                        ...
                        this._map.getCustomPreview(preview.id, preview.part, preview.width, preview.height, preview.tilePosX, preview.tilePosY, preview.tileWidth, preview.tileHeight, { autoUpdate: true });
                    }
                }
            }
        }
    ...
    _getDocumentContainerSize: function () {
        var documentContainerSize = document.getElementById('document-container').getBoundingClientRect();
    ...
    _syncTileContainerSize: function () {
        ...
        if (!this._container)
            return;
        var documentContainerSize = this._getDocumentContainerSize();
    ...
    onAdd: function (map) {
        ...
        this._textCSelections = new CSelections(undefined, this._canvasOverlay, this._selectionsDataDiv, this._map, false /* isView */, undefined, 'text');
        this._cellCSelections = new CSelections(undefined, this._canvasOverlay, this._selectionsDataDiv, this._map, false /* isView */, undefined, 'cell');
        this._oleCSelections = new CSelections(undefined, this._canvasOverlay, this._selectionsDataDiv, this._map, false /* isView */, undefined, 'ole');
    ...
    runZoomAnimation: function (zoomEnd, pinchCenter, mapUpdater, runAtFinish) {
        ...
        var thisObj = this;
        this.zoomStepEnd(zoomEnd, pinchCenter, mapUpdater, 
        // runAtFinish
        function () {
    ...
    _twipsToCorePixels: function (twips) {
        return new L.Point(twips.x * app.twipsToPixels, twips.y * app.twipsToPixels);
    },
    _twipsToCorePixelsBounds: function (twips) {
        return new L.Bounds(this._twipsToCorePixels(twips.min), this._twipsToCorePixels(twips.max));
    },
    _corePixelsToTwips: function (corePixels) {
        return new L.Point(corePixels.x * app.pixelsToTwips, corePixels.y * app.pixelsToTwips);
    },
    _twipsToCssPixels: function (twips) {
        return new L.Point((twips.x / app.tile.size.x) * (TileManager.tileSize / app.dpiScale), (twips.y / app.tile.size.y) * (TileManager.tileSize / app.dpiScale));
    },
    _cssPixelsToTwips: function (pixels) {
        return new L.Point((pixels.x * app.dpiScale) * app.pixelsToTwips, (pixels.y * app.dpiScale) * app.pixelsToTwips);
    },
    ...
    _getMostVisiblePart: function (queue) {
        ...
        var viewedRectangle = app.file.viewedRectangle.pToArray();
    ...
    highlightCurrentPart: function (part) {
        ...
        for (var i = 0; i < previews.length; i++) {
            var img = previews[i].querySelector('img');
    ...
    _sendClientVisibleArea: function (forceUpdate) {
        ...
        visibleArea = new L.Bounds(this._pixelsToTwips(visibleArea.min), this._pixelsToTwips(visibleArea.max));
...
//# sourceMappingURL=CanvasTileLayer.js.map