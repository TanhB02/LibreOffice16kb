...
L.TextInput = L.Layer.extend({
    ...
    _onFocusBlur: function (ev) {
        ...
        onoff(this._textArea, 'copy cut compositionstart compositionupdate compositionend select keydown keypress keyup beforeinput textInput textinput input', this._onEvent, this);
    ...
    _initLayout: function () {
        ...
        this._emptyArea();
        if (!this.hasAccessibilitySupport()) {
            var warningMessage = _('Screen reader support for text content is disabled. ') +
                ...
                _('Also check the voice over toggle under {parentControl}.').replace('{parentControl}', window.userInterfaceMode === 'notebookbar' ? _('the Help tab') : _('the View menu'));
    ...
    _fancyLog: function (type, payload) {
        ...
        if (this._isDebugOn) {
            ...
            window.app.console.log(+new Date() + ' %cINPUT%c: ' + state
                + '"' + content + '" ' + type + '%c ', 'background:#bfb;color:black', 'color:green', 'color:black', JSON.stringify(payload));
        }
    },
    ...
    _onInput: function (ev) {
        ...
        if (this._map.getDocType() === 'text') {
            var contentStr = this.codePointsToString(content);
            var newPara = contentStr.length === 1;
    ...
    _handleKeyDownForPopup: function (ev, id) {
        ...
        if (popup) {
            var entries = document.querySelectorAll('#' + id + ' span.ui-treeview-cell');
            if (ev.key === 'ArrowDown') {
                var initialFocusElement = entries[0];
                ...
                var event_1 = new KeyboardEvent('keydown', {
                    key: 'Enter',
                    bubbles: true,
                    cancelable: true,
                });
                entries[0].dispatchEvent(event_1);
                ev.preventDefault();
                ev.stopPropagation();
            }
    ...
    _onKeyDown: function (ev) {
        ...
        if (this.hasAccessibilitySupport()) {
            if ((this._hasAnySelection && !this._isEditingInSelection && this._map.getDocType() !== 'spreadsheet') ||
                ...
                if (!L.Browser.cypressTest) {
                    var allowedKeyEvent = this._map.keyboard.allowedKeyCodeWhenNotEditing[ev.keyCode] ||
                        ev.ctrlKey ||
                        ev.altKey ||
    ...
    _removeTextContent: function (before, after) {
        ...
        app.socket.sendMessage('removetextcontext id=' +
            this._map.getWinId() +
            ' before=' + before +
            ' after=' + after);
    },
    ...
    _sendCompositionEvent: function (text) {
        ...
        if (!window.mode.isMobile() && !window.mode.isTablet() &&
            ...
            if (app.calc.decimalSeparator && this._map.numPadDecimalPressed) { // decimalSeparator is set only for Calc.
                ...
                codes = this._autoCorrectChars[text];
            ...
            this._followMyCursor();
            app.socket.sendMessage('textinput id=' + winId + ' text=' + encodedText);
        }
    },
    ...
    _sendKeyEvent: function (charCode, unoKeyCode, type) {
        ...
        if (this._map.editorHasFocus() || this._hasFormulaBarFocus()) {
            app.socket.sendMessage('key type=' + type + ' char=' + charCode + ' key=' + unoKeyCode + '\n');
        }
        else {
            app.socket.sendMessage('windowkey id=' +
                this._map.getWinId() +
                ' type=' +
                type +
                ' char=' +
        ...
        if (!this.hasAccessibilitySupport()) {
            ...
            if (this._justSwitchedToEditMode && accept && this._isInitialContent()) {
                ...
                if (this._map._docLayer && app.file.textCursor.visible) {
                    ...
                    var center = app.file.textCursor.rectangle.center;
