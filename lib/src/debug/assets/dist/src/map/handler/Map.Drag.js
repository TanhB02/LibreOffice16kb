...
L.Map.Drag = L.Handler.extend({
    ...
    _onPreDrag: function () {
        ...
        if (this._draggable._newPos.x !== pos.x) {
            var clampedX = Math.max(Math.min(org.x, this._draggable._newPos.x), org.x - Math.max(size.x, 0));
        ...
        if (this._draggable._newPos.y !== pos.y) {
            var clampedY = Math.max(Math.min(org.y, this._draggable._newPos.y), org.y - Math.max(size.y, 0));
...
//# sourceMappingURL=Map.Drag.js.map