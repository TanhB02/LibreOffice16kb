...
L.Map.StateChangeHandler = L.Handler.extend({
    ...
    _onStateChanged: function (e) {
        ...
        var commandName = this.ensureUnoCommandPrefix(e.commandName);
        this._items[commandName] = state;
        if (e.commandName === '.uno:CurrentTrackedChangeId') {
            var redlineId = 'change-' + state;
            var annotations = app.sectionContainer.getSectionWithName(L.CSections.CommentList.name);
            if (annotations)
                annotations.selectById(redlineId);
            else
                console.error('_onStateChanged: section "CommentList" missing');
        }
        ...
        if (e.commandName === '.uno:StartWithPresentation' && (state === true || state === 'true')) {
            var startPresentationParam = window.coolParams.get('startPresentation');
    ...
    ensureUnoCommandPrefix: function (unoCmd) {
...
//# sourceMappingURL=Map.StateChanges.js.map