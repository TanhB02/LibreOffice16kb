...
L.Map.Mouse = L.Handler.extend({
    ...
    addHooks: function () {
        this._map.on('mousedown mouseup mouseover mouseout mousemove dblclick trplclick qdrplclick', this._onMouseEvent, this);
    },
    removeHooks: function () {
        this._map.off('mousedown mouseup mouseover mouseout mousemove dblclick trplclick qdrplclick', this._onMouseEvent, this);
    },
    LOButtons: {
        left: 1,
        middle: 2,
    ...
    _onMouseUpOutside: window.touch.mouseOnly(function (e) {
        ...
        if (this._map.dragging.enabled()) {
            ...
            var mousePos = this._map._docLayer._latLngToTwips(this._map.mouseEventToLatLng(e));
            this._map._docLayer._postMouseEvent('buttonup', mousePos.x, mousePos.y, 1, 1, 0);
            var section = app.sectionContainer.getSectionWithName(L.CSections.Scroll.name);
            if (section)
                section.onScrollVelocity({ vx: 0, vy: 0 });
        }
    })
});
//# sourceMappingURL=Map.Mouse.js.map