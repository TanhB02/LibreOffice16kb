...
L.Map.TouchGesture = L.Handler.extend({
    ...
    _onHammer: function (e) {
        ...
        if (e.isFirst) {
            var point = e.pointers[0], containerPoint = this._map.mouseEventToContainerPoint(point), layerPoint = this._map.containerPointToLayerPoint(containerPoint), latlng = this._map.layerPointToLatLng(layerPoint), funcWizardRangeBounds = getFuncWizRangeBounds(this);
            var twipsPoint = this._map._docLayer._latLngToTwips(latlng);
    ...
    _onPress: function (e) {
        if (this._map.uiManager.isUIBlocked())
            return;
        var point = e.pointers[0], containerPoint = this._map.mouseEventToContainerPoint(point), layerPoint = this._map.containerPointToLayerPoint(containerPoint), latlng = this._map.layerPointToLatLng(layerPoint), mousePos = this._map._docLayer._latLngToTwips(latlng);
        var posInTwips = new app.definitions.simplePoint(mousePos.x, mousePos.y);
        if (this._moving) {
            return;
        }
    ...
    _onPanStart: function (e) {
        ...
        if (increasedCellCursor && increasedCellCursor.containsPoint(posInTwips.toArray())) {
            if (!app.calc.cellCursorRectangle.containsPoint(posInTwips.toArray())) {
                var y = posInTwips.y;
                var x = posInTwips.x;
                var heightBuffer = Math.abs(app.calc.cellCursorRectangle.pHeight) * increaseRatio;
...
//# sourceMappingURL=Map.TouchGesture.js.map