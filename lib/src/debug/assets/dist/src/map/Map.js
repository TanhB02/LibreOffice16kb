...
L.Map = L.Evented.extend({
    ...
    initTextInput: function (docType) {
        var hasAccessibilitySupport = window.enableAccessibility && window.prefs.getBoolean('accessibilityState');
    ...
    updateModificationIndicator: function (newModificationTime) {
        ...
        if (this._modIndicatorInitialized && this._lastmodtime) {
            ...
            if (('minSavedMessageTimeoutSecs' in window) && (elapsed < (window.minSavedMessageTimeoutSecs * 1000))) {
                ...
                dateValue = _('Last saved:') + ' ' + dateTime.toLocaleDateString(String.locale, { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
                timeout = 60000;
            }
    ...
    getDesktopCalcZoomCenter: function () {
        ...
        if (app.calc.cellCursorRectangle) {
            var twipsTopLeft = [app.calc.cellCursorRectangle.x1, app.calc.cellCursorRectangle.y1];
            var cursorInBounds = app.file.viewedRectangle.containsPoint(twipsTopLeft);
            if (cursorInBounds) {
                return new ((_a = L.Point).bind.apply(_a, __spreadArray([void 0], __read(twipsTopLeft), false)))();
            }
        }
        if (docLayer._cellSelectionArea) {
            var twipsCenter = docLayer._cellSelectionArea.center;
            var selectionInBounds = app.file.viewedRectangle.containsPoint(twipsCenter);
            if (selectionInBounds) {
                return new ((_b = L.Point).bind.apply(_b, __spreadArray([void 0], __read(twipsCenter), false)))();
            }
        }
    ...
    setDesktopCalcViewOnZoom: function (zoom, animate) {
        ...
        if (!docLayer.options.sheetGeometryDataEnabled || !docLayer.sheetGeometry)
            return false;
        var typing = app.file.textCursor.visible;
        ...
        var docPos = docLayer._painter._getZoomDocPos(zoomCenter, zoomCenter, freePaneBounds, { freezeX: false, freezeY: false }, splitPos, this.getZoomScale(zoom), true);
        ...
        var mapUpdater = function (animationCalculatedNewCenter) {
            if (animationCalculatedNewCenter) {
                _this._resetView(L.latLng(animationCalculatedNewCenter), zoom);
                return;
            }
            _this._resetView(L.latLng(newCenterLatLng), zoom);
        };
        var runAtFinish = function () {
            _this._ignoreCursorUpdate = false;
            if (typing) {
                docLayer.activateCursor();
            }
        };
        if (animate) {
            this._docLayer.runZoomAnimation(zoom, this.unproject(zoomCenter.divideBy(app.dpiScale), this.getZoom()), mapUpdater, runAtFinish);
            return;
        }
    ...
    setZoom: function (zoom, options, animate) {
        ...
        if (this._docLayer && app.file.textCursor.visible && app.file.viewedRectangle.containsPoint(app.file.textCursor.rectangle.center)) {
            ...
            var newCenter = new L.LatLng(curCenter.lat + (caretPos.lat - curCenter.lat) * (1.0 - zoomScale), curCenter.lng + (caretPos.lng - curCenter.lng) * (1.0 - zoomScale));
            mapUpdater = function () {
            ...
            if (animate) {
                ...
                cssBounds.min.x < 0 ? curCenter.lng : caretPos.lng), mapUpdater, runAtFinish);
            }
            else {
                mapUpdater();
                runAtFinish();
        ...
        if (animate) {
            this._docLayer.runZoomAnimation(zoom, 
            // pinchCenter
            curCenter, mapUpdater, runAtFinish);
        }
        else {
            mapUpdater();
            runAtFinish();
    ...
    setZoomAround: function (latlng, zoom) {
        var scale = this.getZoomScale(zoom), viewHalf = this.getSize().divideBy(2), containerPoint = latlng instanceof L.Point ? latlng : this.latLngToContainerPointIgnoreSplits(latlng), centerOffset = containerPoint.subtract(viewHalf).multiplyBy(1 - 1 / scale), newCenter = this.containerPointToLatLngIgnoreSplits(viewHalf.add(centerOffset));
    ...
    getCorePxDocBounds: function () {
        ...
        return new L.Bounds(this._docLayer._cssPixelsToCore(topleft), this._docLayer._cssPixelsToCore(bottomRight));
    },
    ...
    getLayerMaxBounds: function () {
        return L.bounds(this.latLngToLayerPoint(this.options.maxBounds.getNorthWest()), this.latLngToLayerPoint(this.options.maxBounds.getSouthEast()));
    },
    getLayerDocBounds: function () {
        return L.bounds(this.latLngToLayerPoint(this.options.docBounds.getNorthWest()), this.latLngToLayerPoint(this.options.docBounds.getSouthEast()));
    },
    getSize: function () {
    ...
    _isMouseEnteringLeaving: function (e) {
        var target = e.target || e.srcElement, related = e.relatedTarget;
        if (!target) {
            return false;
        }
    ...
    _handleDOMEvent: function (e) {
        app.idleHandler.notifyActive();
        if (!this._docLayer || !this._loaded || !this._enabled || L.DomEvent._skipped(e)) {
            return;
        }
    ...
    _fireDOMEvent: function (target, e, type) {
        if (this.uiManager.isUIBlocked())
            return;
        if (!target.listens(type, true) && (type !== 'click' || !target.listens('preclick', true))) {
            return;
        }
        if (type === 'contextmenu') {
