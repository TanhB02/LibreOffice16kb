...
L.Control.PartsPreview = L.Control.extend({
        ...
        if (docType === 'presentation' || docType === 'drawing') {
            if (!this._previewInitialized) {
                ...
                if (!app.file.fileBasedView) {
                    ...
                    for (var j = 0; j < app.impress.partList.length; j++) {
    ...
    _createPreview: function (i, hashCode) {
        ...
        var imgSize = this._map.getPreview(i, i, this.options.maxWidth, this.options.maxHeight, { autoUpdate: this.options.autoUpdate,
    ...
    _handleDragStart: function (e) {
        // To avoid having to add a new message to move an arbitrary part, let's select the
        // slide that is being dragged.
        var targetNode = (e.target.id.startsWith('preview') ? e.target : e.target.parentNode);
    ...
    _invalidateParts: function () {
        ...
        for (var part = 0; part < this._previewTiles.length; part++) {
            ...
            this._map.getPreview(part, part, this.options.maxWidth, this.options.maxHeight, { autoUpdate: this.options.autoUpdate,
...
//# sourceMappingURL=Control.PartsPreview.js.map