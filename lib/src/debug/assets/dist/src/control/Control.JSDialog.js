...
L.Control.JSDialog = L.Control.extend({
    ...
    clearDialog: function (id) {
        var _this = this;
        var dialogInfo = this.dialogs[id];
        var builder = dialogInfo.builder;
    ...
    close: function (id, sendCloseEvent) {
        if (id !== undefined && this.dialogs[id]) {
            var dialog_1 = this.dialogs[id];
            if (!sendCloseEvent && dialog_1.overlay && !dialog_1.isSubmenu) {
                app.layoutingService.appendLayoutingTask(function () { L.DomUtil.remove(dialog_1.overlay); });
            }
            if (dialog_1.timeoutId)
                clearTimeout(dialog_1.timeoutId);
            if (dialog_1.isPopup)
    ...
    closePopover: function (id, sendCloseEvent) {
        ...
        if (sendCloseEvent) {
            ...
            var popupParent = this.dialogs[id].popupParent;
    ...
    fadeOutDialog: function (instance) {
        if (!instance.id)
            return;
        var dialogInfo = this.dialogs[instance.id];
        if (!dialogInfo)
            return;
        var container = dialogInfo.container;
        app.layoutingService.appendLayoutingTask(function () {
            L.DomUtil.addClass(container, 'fadeout');
            var timeoutId = null;
            var finallyClose = function () {
    ...
    getOrCreateOverlay: function (instance) {
        ...
        if (!overlay) {
            ...
            if (instance.cancellable) {
                ...
                overlay.onclick = function () { _this.close(instance.id, hasToNotifyServer); };
            }
        }
        instance.overlay = overlay;
    },
    isOnlyChild: function (instance) {
        var isMenu = instance.children && instance.children.length
            && instance.children[0].id === '__MENU__';
        var isOnlyChild = instance.children && instance.children.length &&
    ...
    createContainer: function (instance, documentFragment) {
        ...
        if (instance.haveTitlebar) {
            ...
            var titleCloseButtonText = _('Close dialog');
    ...
    addHandlers: function (instance) {
        ...
        if (instance.nonModal && instance.haveTitlebar) {
            instance.titleCloseButton.onclick = function () {
                var newestDialog = Math.max.apply(null, Object.keys(instance.that.dialogs).map(function (i) { return parseInt(i); }));
        ...
        var tabControlWidget = this.findTabControl(instance);
        var focusWidget, firstFocusableElement;
        if (tabControlWidget) {
        ...
        if (obj.children && obj.children.length > 0) {
            try {
                for (var _b = __values(obj.children), _c = _b.next(); !_c.done; _c = _b.next()) {
                    var child = _c.value;
                    var result = this.findTabControl(child); // Recursively search in children
                    if (result)
                        return result; // If found, return the tabcontrol
                }
            }
        ...
        if (instance.isSnackbar) {
            ...
            if (instance.clickToCloseId && parent) {
                ...
                if ((matchingElements = parent.querySelectorAll('span.ui-treeview-cell-text')).length) { // treeview entry for context menu
                    parent = Array.from(matchingElements).find(function (value) { return value.innerText === instance.clickToCloseText; });
                }
                else if ((matchingElements = parent.querySelectorAll('div.ui-iconview-entry > img')).length) { // iconview entry for context menu
                    parent = Array.from(matchingElements).find(function (img) { return img.title === instance.clickToCloseText; });
                }
            }
    ...
    calculateAutoFilterPosition: function (instance) {
        ...
        var devicePixelRatio = app.dpiScale;
    ...
    getAutoPopupParentContainer: function (instance) {
        ...
        */
        ...
        if (instance.action === 'fadeout') {
            ...
            if (dialogs.length) {
                ...
                var lastDialog = this.dialogs[lastKey];
                var lastContainer = lastDialog.container;
            ...
            var existingNode_1 = this.dialogs[instance.id];
            if (existingNode_1) {
                instance.posx = existingNode_1.startX;
                instance.posy = existingNode_1.startY;
            }
            ...
            var dialogDomParent_1 = instance.overlay ? instance.overlay : instance.containerParent;
            var documentFragment = new DocumentFragment(); // do not modify dom yet
            ...
            app.layoutingService.appendLayoutingTask(function () {
                ...
                if (existingNode_1) {
                    existingNode_1.container.replaceWith(instance.container);
                }
                else {
                    instance.container.classList.add('fadein');
                    dialogDomParent_1.append(instance.container);
                }
                // do in task to apply correct focus when already shown
                _this.addHandlers(instance);
                ...
                if (instance.nonModal && !(instance.startX && instance.startY)) {
                    _this.centerDialogPosition(instance);
                }
                else {
                    instance.updatePos();
                }
                ...
                if (instance.isAutofilter && !instance.isAutoFillPreviewTooltip)
                    _this.calculateAutoFilterPosition(instance);
                else if (instance.isAutoFillPreviewTooltip || instance.isAutoCompletePopup) {
                    _this.updateAutoPopPosition(instance.container, instance.posx, instance.posy);
                }
            });
    ...
    onJSUpdate: function (e) {
        ...
        app.layoutingService.appendLayoutingTask(function () {
            ...
            if (!dialogInfo) {
                ...
                if (dialog)
                    console.debug('JSDialog: old data was: ' + JSON.stringify(dialog));
                return;
            }
        ...
        if (innerData.action_type === 'grab_focus') {
            app.layoutingService.appendLayoutingTask(function () {
                ...
                if (controlPosition && (controlPosition.bottom > window.innerHeight ||
                    ...
                    _this.centerDialogPosition(dialog); // will center it
                }
            });
        }
    ...
    setNewPosition: function (target, newX, newY) {
...
//# sourceMappingURL=Control.JSDialog.js.map