...
L.Map.include({
	...
	setPermission: function (perm) {
		...
		console.log('[PERMISSION] :', "11111");
			button.css('display', 'flex');
		} else {
		console.log('[PERMISSION] :', "22222");
			button.hide();
		}
		var that = this;
        if (perm === 'edit') {
            ...
            if (this._shouldStartReadOnly() || window.mode.isMobile() || window.mode.isTablet()) {
                button.on('click', function () {
                    console.log('[PERMISSION] :', "33333");
                    if (!L.Map.alwaysEditMode) {
                        that._switchToEditMode();
                        L.Map.alwaysEditMode = true;
                    } else {
                        that._enterReadOnlyMode('readonly');
                        L.Map.alwaysEditMode = false; // 👈 reset nếu muốn lần sau lại edit
                    }
                });

                console.log('[PERMISSION] :', "44444");
                if (!L.Map.alwaysEditMode) {
			...
			console.log('[PERMISSION] :', "777777");
				button.on('click', function () {
				console.log('[PERMISSION] :', "88888");
					that.openUnlockPopup();
				});
			}
			else if (window.ThisIsTheAndroidApp) {
			console.log('[PERMISSION] :', "999999");
				button.on('click', function () {
				console.log('[PERMISSION] :', "110101010");
			...
			console.log('[PERMISSION] :', "1212121212");
				$('#mobile-edit-button').hide();
			}

			console.log('[PERMISSION] :', "13131321313");
	...
	_proceedEditMode: function() {
		...
		if (window.mode.isMobile() || window.mode.isTablet()) {
			...
			// Bật bàn phím khi chuyển sang chế độ edit trên mobile/tablet
			this.focus();
		}
	},
	...
	_enterEditMode: function (perm) {
		console.log('[PERMISSION] Entering edit mode:', perm);
	...
	_enterReadOnlyMode: function (perm) {
        console.log('[PERMISSION] Entering read-only mode:', perm, '(ending edit mode)');
		this._permission = perm;
