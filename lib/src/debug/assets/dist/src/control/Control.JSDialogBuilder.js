...
L.Control.JSDialogBuilder = L.Control.extend({
    ...
    baseSpinField: function (parentContainer, data, builder, customCallback) {
        ...
        spinfield.addEventListener('change', function () {
            var isCurrentlyDisabled = div.hasAttribute('disabled');
        ...
        if (data.children.length > 0) {
            ...
            if (data.children[0].text && data.children[0].text !== '') {
                ...
                var toggleFunction = function () {
                    ...
                    var currentState = expander.getAttribute('aria-expanded') === 'true';
    ...
    _findNextElementInContainer: function (container, currentElement, direction) {
        var boundingRectangle = currentElement.getBoundingClientRect();
        var startX = boundingRectangle.left + (boundingRectangle.right - boundingRectangle.left) / 2;
        var startY = boundingRectangle.top + (boundingRectangle.bottom - boundingRectangle.top) / 2;
        var diffX = 0;
        var diffY = 0;
            ...
            if (builder.wizard) {
                ...
                var findNextVisibleTab = function (tab, backwards) {
                    ...
                    for (var i = 1; i <= total; i++) {
                        var nextIndex = (currentIndex + diff * i + total) % total;
                ...
                tabPage.addEventListener('keydown', function (e) {
                    ...
                    if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(key)) {
                        ...
                        if (!(currentElement.tagName === 'INPUT' || currentElement.tagName === 'TEXTAREA')) {
                            if (e.key === 'Tab')
                                e.preventDefault();
                            var container = document.getElementsByClassName('ui-tabs-content notebookbar');
                            var elementToFocus = this._findNextElementInContainer(container[0], currentElement, key);
                            if (elementToFocus && elementToFocus.tagName !== 'NAV')
                                ...
                                var visibleContainer = Array.from(container[0].children).find(function (child) {
                                    return !child.classList.contains('hidden') && child.offsetParent !== null;
                                });
                                var focusables = visibleContainer ? Array.from(visibleContainer.querySelectorAll('[tabindex="-1"]:not([disabled])')) : [];
    ...
    _pushbuttonControl: function (parentContainer, data, builder, customCallback) {
        ...
        var tooltipText = (data.aria && data.aria.label) || data.text;
    ...
    _addAriaLabel: function (element, data, builder) {
        if (data.aria)
    ...
    _createIdentifiable: function (type, classNames, parentContainer, data) {
        return L.DomUtil.create(type, classNames + this._getParentId(data), parentContainer);
    },
    ...
    _unoToolButton: function (parentContainer, data, builder, options) {
        ...
        if (data.command || data.postmessage === true) {
            ...
            var tooltip = builder._cleanText(data.tooltip) || builder._cleanText(data.text);
    ...
    executeAction: function (container, data) {
        var _this = this;
        app.layoutingService.appendLayoutingTask(function () { _this.executeActionImpl(container, data); });
    },
    ...
    updateWidget: function (container, data) {
        var _this = this;
        app.layoutingService.appendLayoutingTask(function () {
            _this._updateWidgetImpl(container, data, _this.build);
        });
    },
...
//# sourceMappingURL=Control.JSDialogBuilder.js.map