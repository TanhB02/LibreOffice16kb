...
L.Control.Notebookbar = L.Control.extend({
    ...
    setTabs: function (tabs) {
        ...
        try {
            for (var _b = __values(tabs.children), _c = _b.next(); !_c.done; _c = _b.next()) {
                var tab = _c.value;
                if (tab.id.endsWith('-tab-label')) {
                    var name_1 = tab.id.substring(0, tab.id.length - 10);
                    if (!this.map.uiManager.isTabVisible(name_1)) {
                        $(tab).hide();
                    }
                }
            }
    ...
    shouldIgnoreContextChange: function (contexts, appId) {
        // New -> old context name pairs.
        var ignored = [['NotesPage', 'DrawPage'], ['DrawPage', 'NotesPage']];
    ...
    updateButtonVisibilityForContext: function (context, tabId) {
        var _this = this;
        var tabsJSON = this.getTabsJSON();
        var splitTabId = tabId.split('-');
        if (splitTabId.length !== 3)
            return;
        var tabName = splitTabId[0];
        var toShow = [];
        var toHide = [];
        tabsJSON.forEach(function (tabContent) {
            if (!tabContent || !tabContent.children[0] || !tabContent.children[0].children)
                return;
            var tabPageId = tabContent.children[0].id;
            var tabPageName = tabPageId.split('-')[0];
            if (tabPageName !== tabName)
                return;
            var children = tabContent.children[0].children;
            var requiredContext = context || 'default';
        ...
        toHide.forEach(function (item) {
            _this.showButton(item, false);
        });
        toShow.forEach(function (item) {
            _this.showButton(item, true);
        });
    },
    ...
    updateTabsVisibilityForContext: function (requestedContext) {
        ...
        var currentlySelectedTabName = null;
        for (var tab in tabs) {
        ...
        if (alreadySelected) {
            var tabId = alreadySelected.attr('id');
        ...
        if (contextTab) {
            ...
            var tabId = contextTab.attr('id');
            this.updateButtonVisibilityForContext(requestedContext, tabId);
            return;
        }
        if (defaultTab) {
            defaultTab.click();
            var tabId = defaultTab.attr('id');
            this.updateButtonVisibilityForContext(requestedContext, tabId);
            return;
        }
    },
...
//# sourceMappingURL=Control.Notebookbar.js.map