...
L.Map.include({
    ...
    sendUnoCommand: function (command, json, force) {
        ...
        if (app.isCommentEditingAllowed()) {
            allowedCommands.push('.uno:InsertAnnotation', '.uno:DeleteCommentThread', '.uno:DeleteAnnotation', '.uno:DeleteNote', '.uno:DeleteComment', '.uno:ReplyComment', '.uno:ReplyToAnnotation', '.uno:PromoteComment', '.uno:ResolveComment', '.uno:ResolveCommentThread', '.uno:ResolveComment', '.uno:EditAnnotation', '.uno:ExportToEPUB', '.uno:ExportToPDF', '.uno:ExportDirectToPDF');
            var graphicInfo = GraphicSelection.extraInfo;
    ...
    onHelpOpen: function (id, map, productName) {
        ...
        var onlineHelpContent = document.getElementById('online-help-content');
        var buttons = onlineHelpContent.querySelectorAll('.scroll-button');
    ...
    _doOpenHelpFile: function (data, id, map) {
        ...
        app.layoutingService.appendLayoutingTask(function () {
            ...
            _this.onHelpOpen(id, map, productName);
        });
    },
    ...
    _createAndRunHyperlinkDialog: function (defaultText, defaultLink) {
        ...
        var focusId = 'hyperlink-link-box-input';
        if (defaultText === '') {
