...
var StatusBar = /** @class */ (function (_super) {
    ...
    function StatusBar(map) {
        var _this = _super.call(this, map, 'toolbar-down') || this;
        map.on('doclayerinit', _this.onDocLayerInit, _this);
        map.on('languagesupdated', _this.onLanguagesUpdated, _this);
        map.on('commandstatechanged', _this.onCommandStateChanged, _this);
        app.events.on('contextchange', _this.onContextChange.bind(_this));
        app.events.on('updatepermission', _this.onPermissionChanged.bind(_this));
        map.on('updatestatepagenumber', _this.onPageChange, _this);
        map.on('search', _this.onSearch, _this);
        map.on('zoomend', _this.onZoomEnd, _this);
        map.on('initmodificationindicator', _this.onInitModificationIndicator, _this);
        map.on('updatemodificationindicator', _this.onUpdateModificationIndicator, _this);
        return _this;
    }
    StatusBar.prototype.isSaveIndicatorActive = function () {
        return window.useStatusbarSaveIndicator;
    };
    StatusBar.prototype.localizeStateTableCell = function (text) {
    ...
    StatusBar.prototype.toLocalePattern = function (pattern, regex, text, sub1, sub2) {
    ...
    StatusBar.prototype._updateToolbarsVisibility = function (context) {
    ...
    StatusBar.prototype.onContextChange = function (event) {
        this._updateToolbarsVisibility(event.detail.context);
    };
    StatusBar.prototype.callback = function (objectType, eventType, object, data, builder) {
        if (object.id === 'search-input' || object.id === 'search') {
            // its handled by widget itself
            return;
        }
        else if (object.id === 'zoom') {
            var selected = this._generateZoomItems().filter(function (item) { return item.id === data; });
            if (selected.length)
    ...
    StatusBar.prototype.onSearch = function (e) {
    ...
    StatusBar.prototype.onZoomEnd = function () {
        var zoomPercent = 100;
    ...
    StatusBar.prototype._generateHtmlItem = function (id, dataPriority) {
    ...
    StatusBar.prototype._generateStateMenuEntry = function (id, text, selection) {
        return { id: id, text: text, selected: !!(selection & parseInt(id)) };
    };
    StatusBar.prototype._generateStateTableCellMenuItem = function (selection, visible) {
        var submenu = [
    ...
    StatusBar.prototype._generateZoomItems = function () {
        return [
    ...
    StatusBar.prototype.getToolItems = function () {
        return [
            ...
            this._generateHtmlItem('statusdocpos'),
            this._generateHtmlItem('rowcolselcount', 1),
            this._generateHtmlItem('statepagenumber'),
            this._generateHtmlItem('statewordcount', 1),
            this._generateHtmlItem('insertmode', 5),
            this._generateHtmlItem('showcomments', 4),
            this._generateHtmlItem('statusselectionmode', 6),
            this._generateHtmlItem('slidestatus'),
            this._generateHtmlItem('pagestatus'),
            ...
            this._generateHtmlItem('statetablecell', 4),
            this._generateStateTableCellMenuItem(2, false),
            { type: 'separator', id: 'statetablebreak', orientation: 'vertical', visible: false, dataPriority: 7 },
            ...
            this._generateHtmlItem('documentstatus', 2),
    ...
    StatusBar.prototype.onDocLayerInit = function () {
    ...
    StatusBar.prototype.updateHtmlItem = function (id, text, disabled) {
        this.builder.updateWidget(this.parentContainer, {
            id: id,
            type: 'htmlcontent',
    ...
    StatusBar.prototype.updateLanguageItem = function (language) {
        if (app.map.isReadOnlyMode())
            return;
        this.builder.updateWidget(this.parentContainer, { type: 'menubutton', id: 'languagestatus:LanguageStatusMenu', noLabel: false, text: language });
        JSDialog.RefreshScrollables();
    };
    StatusBar.prototype.showSigningItem = function (icon, text) {
        this.builder.updateWidget(this.parentContainer, { type: 'toolitem', id: 'signstatus', command: '.uno:Signature', w2icon: icon, text: text ? text : _UNO('.uno:Signature') });
        JSDialog.RefreshScrollables();
    };
    StatusBar.prototype.onPermissionChanged = function (event) {
    ...
    StatusBar.prototype.extractLanguageFromStatus = function (state) {
        var code = state;
    ...
    StatusBar.prototype.onCommandStateChanged = function (e) {
    ...
    StatusBar.prototype.onLanguagesUpdated = function () {
        var menuEntries = [];
    ...
    StatusBar.prototype.updateDefaultStateAttribute = function () {
        var docstatcontainer = document.getElementById('documentstatus-container');
    ...
    StatusBar.prototype.onInitModificationIndicator = function (lastmodtime) {
    ...
    StatusBar.prototype.onUpdateModificationIndicator = function (e) {
        if (!this.isSaveIndicatorActive())
            return;
...
//# sourceMappingURL=Control.StatusBar.js.map