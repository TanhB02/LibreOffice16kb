...
JSDialog.combobox = function (parentContainer, data, builder) {
    ...
    content.addEventListener('keyup', function (event) {
        var shouldTriggerChange = data.changeOnEnterOnly ? event.key === 'Enter' : true;
        ...
        if (event.key === 'ArrowDown' && builder.map.jsdialog.hasDropdownOpened()) {
            var comboboxEntries = JSDialog.GetDropdown(data.id);
            var selectedElement = comboboxEntries.querySelector(".selected");
            if (selectedElement) {
...
//# sourceMappingURL=Widget.Combobox.js.map