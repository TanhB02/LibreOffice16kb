...
var Tooltip = /** @class */ (function () {
    function Tooltip(options) {
        ...
        var win = this._options.window ? this._options.window : window;
        this._container = this._options.container
            ? this._options.container
            : L.DomUtil.create('div', 'cooltip-text', win.document.body);
        this._container.id = 'cooltip';
        this._container.addEventListener('mouseenter', L.bind(this.mouseEnter, this));
        this._container.addEventListener('mouseleave', L.bind(this.mouseLeave, this));
        win.addEventListener('keydown', L.bind(this.keyDown, this), {
            capture: true,
            passive: true,
        });
    }
    Tooltip.prototype.beginShow = function (elem) {
        if (this._cancel)
            return;
        var win = this._options.window ? this._options.window : window;
        win.clearTimeout(this._showTimeout);
        this._showTimeout = win.setTimeout(L.bind(this.show, this, elem), this._options.timeout);
    };
    Tooltip.prototype.beginHide = function (elem) {
        if (this._cancel)
            return;
        var win = this._options.window ? this._options.window : window;
        win.clearTimeout(this._showTimeout);
        ...
        if (this._current)
            this._hideTimeout = win.setTimeout(L.bind(this.hide, this, elem), this._options.timeout);
    };
    /**
     ...
     */
    Tooltip.prototype.position = function (elem, popup, index) {
    ...
    Tooltip.prototype.show = function (elem, textContent) {
        // `textContent` adds flexibility, enabling custom messages like document "Saved" instead of the fixed "cool-tooltip."
        var content = textContent ? textContent : elem.dataset.cooltip, rectView = new DOMRect(0, 0, window.innerWidth, window.innerHeight), rectElem = elem.getBoundingClientRect(), rectCont, rectTooltip, index = 0;
        this._container.textContent = content;
        if (!this._container.textContent)
            return;
    ...
    Tooltip.attachEventListener = function (elem, map) {
        if (!map.tooltip) {
            return;
        }
...
//# sourceMappingURL=Control.Tooltip.js.map