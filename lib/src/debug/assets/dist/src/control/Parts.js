...
L.Map.include({
    ...
    */
    ...
    selectPart: function (part, how, external, fireEvent) {
        if (fireEvent === void 0) { fireEvent = true; }
        var currentSelectedCount = app.impress.getSelectedSlidesCount();
        var targetPart = app.impress.partList[part];
        if (how < 2)
            targetPart.selected = how;
        else
            targetPart.selected = targetPart.selected === 1 ? 0 : 1;
    ...
    getPreview: function (id, part, maxWidth, maxHeight, options) {
        ...
        var fetchThumbnail = options && options.fetchThumbnail !== undefined ? options.fetchThumbnail : true;
        var isSlideshow = options && options.slideshow !== undefined ? options.slideshow : false;
        ...
        if (docLayer._docType === 'text')
            return;
        var tileWidth = docLayer._partWidthTwips ? docLayer._partWidthTwips : app.file.size.x;
        var tileHeight = docLayer._partHeightTwips ? docLayer._partHeightTwips : app.file.size.y;
    ...
    goToPage: function (page) {
        ...
        if (!this.isEditMode() && app.file.writer.pageRectangleList.length > docLayer._currentPage) {
            var posY = Math.round(app.file.writer.pageRectangleList[docLayer._currentPage][1] / app.dpiScale);
    ...
    showPage: function () {
        if (this.getDocType() === 'spreadsheet' && app.calc.isAnyPartHidden()) {
            var dialogId = 'show-sheets-modal';
    ...
    showPageModalImpl: function (dialogId) {
        ...
        if (!modal) {
            setTimeout(function () { _this.showPageModalImpl(dialogId); }, 10);
            return;
        }
