...
L.Control.Zotero = L.Control.extend({
    ...
    citations: {},
    itemsPerPage: 15,
    getCachedOrFetch: function (url, retry) {
    ...
    showUnsupportedItemWarning: function (uriList) {
        ...
        for (var i in uriList) {
            ...
            if (!this.groupIdList.has(id) && id !== this.userID.toString()) {
                ...
                this.map.uiManager.showInfoModal('zoterounreachablewarn', _('Zotero Warning'), _('The document contains some citations which may be unreachable through web API. It may cause some problems while editing citations or bibliography.'), null, _('Close'), function () { });
                return;
            }
        }
    },
            ...
            return {
                data: {
                    ...
                    control: {
                        id: 'zoterolist',
                        type: 'treelistbox',
                        headers: headerArray.map(function (item) { return { text: item }; }),
                        entries: this.items,
                    },
                },
                callback: this._onAction.bind(this)
    ...
    createEntry: function (index, columns, entryData, hasIcon) {
        ...
        this.items.push(Object.assign({ 'columns': firstColumn.concat(columns.map(function (item) {
                return { text: item };
    ...
    fillItems: function (items) {
        ...
        for (var iterator = 0; iterator < items.length; ++iterator) {
            ...
            this.createEntry(index++, [items[iterator].csljson.title, creatorString, items[iterator].data.date ? items[iterator].data.date : items[iterator].data.dateDecided], { type: 'item', itemType: items[iterator].data.itemType, item: items[iterator] }, true);
        }
    },
    fillStyles: function (styles) {
        ...
        for (var iterator = 0; iterator < styles.length; ++iterator) {
            this.createEntry(iterator, [styles[iterator].title], Object.assign({ name: styles[iterator].name, type: 'style' }, styleToSelect === styles[iterator].name ? { selected: true } : null));
        }
    },
    fillNotes: function (items, notesCount) {
        ...
        for (var iterator = 0; iterator < items.length; ++iterator) {
            ...
            this.createEntry(index++, [note], { type: 'note', itemType: items[iterator].data.itemType, item: items[iterator] }, true);
        }
        return index;
    },
    ...
    makeLanguageAvailable: function (locale) {
        var ret = null;
        var languages = Object.keys(this.languageNames).filter(function (value) { return value.indexOf(locale) >= 0; });
    ...
    checkStyleTypeAndEnableOK: function (selectedStyle) {
        ...
        fetch('https://www.zotero.org/styles/' + style)
            ...
            var availableLocale = Object.keys(that.availableLanguages).indexOf(that.settings.locale) >= 0;
            that.enableDialogLanguageCombobox(availableLocale ? that.settings.locale : defaultLocale);
            var categories = csl.getElementsByTagName('category');
            for (var i = 0; i < categories.length; i++) {
                if (categories[i].getAttribute('citation-format')) {
    ...
    _onAction: function (element, action, data, index) {
        if (element === 'dialog' && action === 'close')
            return;
        if (element === 'treeview') {
            if (action == 'keydown')
                return;
        ...
        if (element === 'edit' && data.id === 'zoterosearch') {
            var query = data.querySelector('input').value;
            if (query)
        ...
        if (data.id == 'ok') {
            ...
            if (!this.selected || this.selected.type === 'style') {
                ...
                this.map.uiManager.showConfirmModal('zoterofieldtypewarn', _('Citation warning'), _('Once citations are entered their storage and display type can not be changed.'), _('Confirm'), function () {
    ...
    unlinkCitations: function () {
        this.map.uiManager.showInfoModal('zoterounlinkwarn', _('Unlink Citations'), _('Unlinking citations will prevent Collabora Online from updating citations and bibliography in this document.'), null, _('Confirm'), this.handleUnlinkCitations.bind(this));
    },
...
//# sourceMappingURL=Control.Zotero.js.map