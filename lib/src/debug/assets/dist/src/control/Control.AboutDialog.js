...
var AboutDialog = /** @class */ (function () {
    function AboutDialog(map) {
        this.map = map;
    }
    AboutDialog.prototype.aboutDialogClickHandler = function (e) {
        if (e.detail === 3) {
            this.map._debug.toggle();
        }
    };
    AboutDialog.prototype.adjustIDs = function (content) {
        var servedBy = content.querySelector(':scope #served-by');
        if (servedBy)
            servedBy.id += '-cloned';
        var wopiHostId = content.querySelector(':scope #wopi-host-id');
        if (wopiHostId)
            wopiHostId.id += '-cloned';
    };
    AboutDialog.prototype.hideElementsHiddenByDefault = function (content) {
        var servedBy = content.querySelector(':scope #served-by-cloned');
        if (servedBy)
            servedBy.style.display = 'none';
        var wopiHostId = content.querySelector(':scope #wopi-host-id-cloned');
        if (wopiHostId)
    ...
    AboutDialog.prototype.show = function () {
        ...
        */
        ...
        if (content.querySelector('#js-dialog')) {
            content.querySelector('#js-dialog').onclick =
                function () {
        ...
        var productURL = typeof brandProductURL === 'string' && brandProductURL.length > 0
            ? brandProductURL
            : 'https://collaboraonline.github.io/';
        var productNameElement = content.querySelector('#product-name');
        ...
        var productStringElement = content.querySelector('#product-string');
        if (productStringElement)
            productStringElement.innerText = productString.replace('{productname}', productNameWithURL);
        var slowProxyElement = content.querySelector('#slow-proxy');
        if (windowAny.socketProxy)
            slowProxyElement.innerText = _('"Slow Proxy"');
        if (windowAny.indirectSocket) {
            var routeTokenElement = content.querySelector('#routeToken');
            routeTokenElement.innerText = 'RouteToken: ' + windowAny.routeToken;
            if (windowAny.geolocationSetup) {
                var timezoneElement = content.querySelector('#timeZone');
                timezoneElement.innerText =
                    _('Time zone:') + ' ' + app.socket.WSDServer.TimeZone;
            }
        }
        this.map.uiManager.showYesNoButton(aboutDialogId + '-box', productName, '', _('OK'), null, null, null, true);
        this.showImpl(aboutDialogId, content);
    };
    AboutDialog.prototype.showImpl = function (aboutDialogId, content) {
        var _this = this;
        var box = document.getElementById(aboutDialogId + '-box');
        // TODO: do it JSDialog native...
        if (!box) {
            setTimeout(function () {
                _this.showImpl(aboutDialogId, content);
            }, 10);
            return;
        }
        ...
        var copyVersionText = _('Copy all version information in English');
    ...
    AboutDialog.prototype.aboutDialogKeyHandler = function (e) {
        if (e.key === 'd') {
            this.map._debug.toggle();
        }
        else if (e.key === 'l') {
            ...
            app.socket.threadLocalLoggingLevelToggle =
                !app.socket.threadLocalLoggingLevelToggle;
            var newLogLevel = app.socket.threadLocalLoggingLevelToggle
                ? 'verbose'
                : 'default';
            app.socket.sendMessage('loggingleveloverride ' + newLogLevel);
            var logLevelInformation = void 0;
    ...
    AboutDialog.prototype.copyVersionInfoToClipboard = function () {
        var text = 'COOLWSD version: ' +
    ...
    AboutDialog.prototype.contentHasBeenCopiedShowSnackbar = function () {
        var _this = this;
        var timeout = 1000;
        this.map.uiManager.showSnackbar('Version information has been copied', null, null, timeout);
        ...
        setTimeout(function () {
            app.LOUtil.setImage(copybutton, 'lc_copy.svg', _this.map);
        }, timeout);
    };
    AboutDialog.prototype.getVersionInfoFromClass = function (className) {
...
// Initiate the class.
JSDialog.aboutDialog = function (map) {
    return new AboutDialog(map);
};
//# sourceMappingURL=Control.AboutDialog.js.map